// Package clock provides a small time abstraction so that state transitions and
// lease expiry can be driven deterministically in tests. Production code uses
// Real(); tests use a Fake whose Advance method fires scheduled callbacks
...
// Clock is the subset of the time package that mudflaps depends on.
